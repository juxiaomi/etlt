...
//	@Test
	public void testPriority() {
		...
		for (String expression : expressions) {
			...
			System.out.println(getExpressionEvaluator().compile(expression));
			Object result = getExpressionEvaluator().evaluate(expression);
...
//	@Test
	public void testMess() {
		...
		expressions.add(
				"$CONTAINS('aabbcc' + 'abc','abc') ? \r\n$STARTSWITH('abcbcc','abc') : \r\n$ENDSWITH('aabcbcc','abc') ");
		expressions
				.add("$CONTAINS('aabbcc' , 'abc') ?\r\n $STARTSWITH('abcbcc','abc') \r\n: $ENDSWITH('aabcbcc','abc') ");
