...
public abstract class BaseDataMeta {
	...
	public Boolean getBooleanValue() {
		if (DataType.DATATYPE_BOOLEAN != this.dataType) {
			throw new UnsupportedOperationException("The current constant type does not support this operation");
		}
	...
	/**
	 * Gets the collection object of the data
	 * 
	 * @return
	 */
	...
	/**
	 * Gets the reference object of the token
	 * 
	 * @return
	 */
	...
	/**
	 * Verify the validity of data type and value
	 */
	protected void verifyDataMeta() {
		if (dataType != null && dataValue != null) {
			if (DataType.DATATYPE_UNKNOWN == dataType && dataValue != null) {
	...
	public Class<?> mapTypeToJavaClass() {
		...
		} else if (BaseDataMeta.DataType.DATATYPE_UNKNOWN == this.getDataType()) {
			return null;

		}
		throw new IllegalExpressionException("Unrecognized data type");
	}

	/**
	 * Check that the compatibility types of data types are the same,
	 * but some compatibility types are different,
	 * then the compatible data types include int, long, float, double, null, which are compatible with all types
	 * 
	 * @param another
	 * @return
	 */
	private boolean isCompatibleType(BaseDataMeta another) {
		...
		if (DataType.DATATYPE_UNKNOWN == this.getDataType() || DataType.DATATYPE_UNKNOWN == another.getDataType()) {
			return true;
	...
	/**
	 * Gets the compatible type of two numbers. If two data types are incompatible, null is returned
	 * 
	 * @param another
	 * @return
	 */
	public DataType getCompatibleType(BaseDataMeta another) {
		...
		if (isCompatibleType(another)) {
			if (DataType.DATATYPE_UNKNOWN == this.getDataType()) {
				return another.getDataType();

			} else if (DataType.DATATYPE_UNKNOWN == another.getDataType()) {
			...
			throw new IllegalExpressionException("Incompatible data types:[" + this.getDataValue() + "(" + this.getDataType() + ")],["
					+ another.getDataValue() + "(" + another.getDataType() + ")]");
		}
	}
