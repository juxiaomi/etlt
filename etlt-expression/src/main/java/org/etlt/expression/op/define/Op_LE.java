...
public class Op_LE implements IOperatorExecution {
	...
	public Literal verify(int opPositin, BaseDataMeta[] args)
		...
		if(args.length != 2){
			//抛异常
			throw new IllegalExpressionException("operator \"" + THIS_OPERATOR.getToken() + "\" parameter number not match"
		...
		if(first == null || second == null){
			throw new NullPointerException("operator \"" + THIS_OPERATOR.getToken() + "\" parameter missing");
		}		
		...
		if(BaseDataMeta.DataType.DATATYPE_DATE ==  first.getDataType()
			...
			//LE操作不支持其他类型，抛异常
			throw new IllegalExpressionException("operator \"" + THIS_OPERATOR.getToken() + "\" parameter type error"	);
