...
public class Op_SELECT implements IOperatorExecution {
		...
		if( BaseDataMeta.DataType.DATATYPE_BOOLEAN ==  first.getDataType()){
			...
			if(first.getBooleanValue()){
				...
				if(second.isReference()){
					...
					second = secondRef.execute(null);
				}
				Literal result = new Literal(compatibleType , second.getDataValue());
				return result;
			}else{
				//选择第三参数
				//如果第三参数为引用，则执行引用
				if(third.isReference()){
					...
					third = thirdRef.execute(null);
				}				
				Literal result = new Literal(compatibleType , third.getDataValue());
				return result;
			}
	...
	public Literal verify(int opPositin, BaseDataMeta[] args)
		...
		if(compatibleType != null){
			return new Literal(compatibleType, null);
